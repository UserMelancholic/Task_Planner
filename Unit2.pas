...
// Передернем фильтр для Table2
// При этом для каждой записи таблицы будет вызвана процедура Table2FilterRecord
// в которой определено условие отбора записей
...
// Процедура FilterRecord вызывается для каждого записи таблицы при включенном Фильтре.
// Если для текущей записи проверяемое условие выполняется, то неободимо установить значение параметра Accept = True,
// в этом случае эта запись выводится, иначе - не выводится.
...
// итоговое значение Accept для отбора текущей записи определяется по связке AND для всех заданных условий.
...
//    ShowMessage('Param1 = '+AnsiLowerCase(Edit1.Text));
...
//  ShowMessage('1.Accept='+BoolToStr(Accept));

  if Length(Edit2.Text) > 0 then begin  // задан фильтр для поля 'Организация'
//    ShowMessage('Param1 = '+AnsiLowerCase(Edit2.Text));
//    ShowMessage('Param2 = '+AnsiLowerCase(DataSet.FieldValues['Организация']));
//    ShowMessage(IntToStr(AnsiPos(AnsiLowerCase(Edit2.Text), AnsiLowerCase(DataSet.FieldValues['Организация']))));
...
//  ShowMessage('2.Accept='+BoolToStr(Accept));
  if Length(Edit3.Text) > 0 then begin  // задан фильтр для поля 'Моб.телефон'
    if DataSet.FieldValues['Моб.телефон'] = null then  // значение поля не заполнено
      Accept := False   // а фильтр задан по этому полю, поэтому эту запись не отбираем
    else  // дополнительно проверяем содержит ли значение фильтра в значении поля текущей запис
      Accept := Accept and ( Pos(Edit3.Text, DataSet.FieldValues['Моб.телефон']) > 0 )
  end;
  if Length(Edit4.Text) > 0 then begin  // задан фильтр для поля 'Роб.телефон'
    if DataSet.FieldValues['Роб.телефон'] = null then  // значение поля не заполнено
      Accept := False   // а фильтр задан по этому полю, поэтому эту запись не отбираем
    else  // дополнительно проверяем содержит ли значение фильтра в значении поля текущей запис
      Accept := Accept and ( Pos(Edit4.Text, DataSet.FieldValues['Роб.телефон']) > 0 )
  end;
  if Length(Edit5.Text) > 0 then begin  // задан фильтр для поля 'Дом.телефон'
    if DataSet.FieldValues['Дом.телефон'] = null then  // значение поля не заполнено
      Accept := False   // а фильтр задан по этому полю, поэтому эту запись не отбираем
    else  // дополнительно проверяем содержит ли значение фильтра в значении поля текущей запис
      Accept := Accept and ( Pos(Edit5.Text, DataSet.FieldValues['Дом.телефон']) > 0 )
  end;
  if Length(Edit6.Text) > 0 then begin  // задан фильтр для поля 'E-mail'
    if DataSet.FieldValues['E-mail'] = null then  // значение поля не заполнено
      Accept := False   // а фильтр задан по этому полю, поэтому эту запись не отбираем
    else  // дополнительно проверяем содержит ли значение фильтра в значении поля текущей запис
      Accept := Accept and (AnsiPos(AnsiLowerCase(Edit6.Text), AnsiLowerCase(DataSet.FieldValues['E-mail'])) > 0)
  end;
  if Length(Edit7.Text) > 0 then begin  // задан фильтр для поля 'Адрес'
    if DataSet.FieldValues['Адрес'] = null then  // значение поля не заполнено
      Accept := False   // а фильтр задан по этому полю, поэтому эту запись не отбираем
    else  // дополнительно проверяем содержит ли значение фильтра в значении поля текущей запис
      Accept := Accept and (AnsiPos(AnsiLowerCase(Edit7.Text), AnsiLowerCase(DataSet.FieldValues['Адрес'])) > 0)
  end;
//  ShowMessage('Accept='+BoolToStr(Accept));
